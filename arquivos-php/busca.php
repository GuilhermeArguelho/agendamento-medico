...
$sql = "SELECT id, nome_paciente, to_char(data, 'DD/MM/YYYY') as data, to_char(horario, 'HH24:MI') AS horario FROM consulta WHERE nome_paciente 
ilike :nomeBuscado";
...
try {
  ...
  if ($resultado == true) {
    ...
    if($count == 0){
      header("Location: index.php?msgErro=Busca não encontrada.");
    }
  }
  
} catch (PDOException $e) {
  die($e->getMessage());
  header("Location: index.php?msgErro=Falha em realizar a busca.");
}
?>
